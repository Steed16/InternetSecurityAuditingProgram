...
namespace ComputerNetworks
{
    public class ProxyTestController
    {
        ...
        private async Task OnRequest(object sender, SessionEventArgs e)
        {
            ...
            // Filter URL
            if (e.WebSession.Request.RequestUri.AbsoluteUri != Database.BlacklistUrl)
            {
                bool blacklisted = await Database.IsBlacklistedAsync(e.WebSession.Request.RequestUri.Host);
                if (blacklisted)
                {
                    e.Ok("<!DOCTYPE html>" +
                          "<html><body><h1>" +
                          "Website Blocked" +
                          "</h1>" +
                          "<p>Blocked by the POWER SCREEN.</p>" +
                          "</body>" +
                          "</html>");
                }
            }
